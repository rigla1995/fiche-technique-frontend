...
interface Props {
  /** 'page' = pleine page (/client/ai-assistant) · 'widget' = panneau flottant */
  variant?: 'page' | 'widget';
  /** Fermeture du panneau (variante widget) */
  onClose?: () => void;
}

/**
 * Chat de l'assistant IA LabFlow — logique et rendu partagés entre la page
 * dédiée et le widget flottant présent sur toutes les interfaces client/gérant.
 * La conversation est persistée côté serveur (GET/DELETE /conversation).
 */
export default function AssistantChat({ variant = 'page', onClose }: Props) {
  ...
  if (enabled === null || loadingHistory) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: widget ? '100%' : '60vh', color: '#64748b', fontSize: 14 }}>
        Chargement…
      </div>
    );
  }
  ...
  if (!enabled) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: widget ? '100%' : '60vh', gap: 16, textAlign: 'center', padding: 24 }}>
        <div style={{ fontSize: 48 }}>🤖</div>
        <div style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>Assistant IA non activé</div>
        <div style={{ fontSize: 14, color: '#64748b', maxWidth: 380 }}>
          L'assistant IA n'est pas encore activé pour votre compte. Contactez votre administrateur pour l'activer.
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={widget
      ...
      : { display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)', maxWidth: 800, margin: '0 auto', padding: '16px 16px 0' }}>
      {/* En-tête : bandeau dégradé compact (widget) ou en-tête de page */}
      {widget ? (
        <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: 'linear-gradient(135deg,#4f46e5,#8b5cf6)', color: '#fff' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14, fontWeight: 800, lineHeight: 1.2 }}>Assistant IA LabFlow</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexShrink: 0 }}>
          ...
          {messages.length > 0 && (
            <button
              ...
              disabled={clearing}
              style={{ fontSize: 12, color: '#94a3b8', background: 'none', border: 'none', cursor: clearing ? 'not-allowed' : 'pointer', padding: '6px 10px', borderRadius: 8, transition: 'color 0.15s' }}
      ...
      {/* Messages */}
      <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12, paddingBottom: 12, ...(widget ? { padding: '12px 12px 8px' } : {}) }}>
        {messages.length === 0 && !sending && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 12, color: '#94a3b8', textAlign: 'center' }}>
            <div style={{ fontSize: widget ? 32 : 40 }}>💬</div>
      ...
      {/* Input */}
      <div style={{ flexShrink: 0, borderTop: '1px solid #e2e8f0', background: '#fff', ...(widget ? { padding: '8px 12px 10px' } : { paddingBottom: 16, paddingTop: 8 }) }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
          <textarea
            ref={inputRef}
            value={input}
            ...
            disabled={sending}
            placeholder={widget ? 'Posez votre question…' : 'Posez votre question… (Entrée pour envoyer, Maj+Entrée pour nouvelle ligne)'}
            rows={1}
            style={{

  ...
  return (
    <div className="page">
      ...
      <div style={{ background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', padding: '10px 14px', marginBottom: 20, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
        ...
        {presets.map(p => (
          <button key={p.key} onClick={() => setPreset(p.key)} style={{ fontSize: '0.78rem', padding: '5px 11px', borderRadius: 8, cursor: 'pointer', fontWeight: 600, border: `1px solid ${preset === p.key ? '#b45309' : 'var(--border)'}`, background: preset === p.key ? '#fffbeb' : 'var(--bg)', color: preset === p.key ? '#92400e' : 'var(--text)' }}>{p.label}</button>
        ))}
        {preset === 'perso' && (<>
        ...
        <select value={canal} onChange={e => setCanal(e.target.value)} style={{ fontSize: '0.78rem', padding: '6px 10px', borderRadius: 8, border: '1px solid var(--border)' }}>
          <option value="">Tous canaux</option><option value="directe">Direct</option><option value="prestataire">Prestataires</option>
        </select>
      </div>

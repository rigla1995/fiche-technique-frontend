...
interface FamRow { nom: string; consommable: boolean; }
const emptyRow = (): FamRow => ({ nom: '', consommable: true });
...
export default function ReferentielFamillesPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      await Promise.all(valid.map(r => api.post('/api/familles', { nom: r.nom.trim(), consommable: r.consommable })));
      closeCreate(); load();
    } catch (e: unknown) {
  ...
  const openEdit = (f: Famille) => { setEditItem(f); setEditNom(f.name); setEditConsommable(f.consommable); setEditError(''); setSaving(false); };
  const closeEdit = () => setEditItem(null);

  const handleSave = async () => {
    if (!editNom.trim()) { setEditError('Nom requis'); return; }
    setSaving(true);
    try { await api.put(`/api/familles/${editItem!.id}`, { nom: editNom.trim(), consommable: editConsommable }); closeEdit(); load(); }
      ...
      {loading ? (
        ...
        <div className="table-responsive card">
          <table className="table">
            <thead><tr><th>Nom</th><th style={{ textAlign: 'center' }}>Consommable</th><th style={{ textAlign: 'right' }}>Actions</th></tr></thead>
            <tbody>
              {filtered.map(f => (
                <tr key={f.id}>
                  <td>
